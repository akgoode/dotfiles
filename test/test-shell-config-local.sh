#!/bin/bash
# Test zsh configuration loading and functionality (local version)

set -e

echo "========================================"
echo "  Shell Configuration Tests (Local)"
echo "========================================"
...
rm -rf test-repo-shell-config
git init test-repo-shell-config &>/dev/null
cd test-repo-shell-config
...
elif [ -f shell/aliases.zsh ]; then
    echo "    ✓ Aliases file exists (in current directory)"
    # Create symlink for testing
    mkdir -p ~/.config/dotfiles/shell
    ln -sf "$(pwd)/shell/aliases.zsh" ~/.config/dotfiles/shell/aliases.zsh
else
